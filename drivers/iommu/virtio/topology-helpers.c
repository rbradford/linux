...
static bool virt_topo_device_match(struct device *dev,
				   struct virt_topo_dev_id *id, u32 *epid_base)
{
...
static const struct iommu_ops *virt_iommu_setup(struct device *dev)
{
	...
	list_for_each_entry(ep, &endpoints, list) {
		if (virt_topo_device_match(dev, &ep->dev_id, &epid)) {
			epid += ep->endpoint_id;
			viommu = ep->viommu;
			break;
		}
	...
	if (virt_topo_device_match(dev, &viommu->dev_id, &epid) ||
	    dev == viommu->dev)
		return NULL;
...
void virt_topo_set_iommu_ops(struct device *dev, struct iommu_ops *ops)
{
	...
	list_for_each_entry(viommu, &viommus, list) {
		 ...
		 */
		if (!viommu->dev &&
		    virt_topo_device_match(dev, &viommu->dev_id, &epid))
			viommu->dev = dev;
